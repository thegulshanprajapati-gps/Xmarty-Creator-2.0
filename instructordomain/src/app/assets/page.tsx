...
export default function AssetsPage() {
  ...
  const handleCreateFolder = async (name: string, description: string, isPaid: boolean, parentId: string | null) => {
    ...
    showAlert("Creating empty directories on Cloudinary directly is not supported. Please upload an asset inside the folder structure instead.", "Unsupported Action");
  };

  const handleRenameItem = async (id: string, isFolder: boolean, newName: string) => {
    showAlert("Renaming Cloudinary files directly is not supported in this view.", "Unsupported Action");
  };
  ...
  const handleUploadFile = async (file: File, parentFolderId: string | null) => {
    ...
    try {
      ...
      showAlert(`Upload failed: ${error.message}`, "Upload Error");
    } finally {
      setLoading(false);
    }
  };
