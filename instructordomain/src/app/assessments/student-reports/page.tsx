...
export default function StudentReportsPage() {
  ...
  const fetchData = async () => {
    setLoading(true);
    try {
      const [attemptsRes, testRes, userRes] = await Promise.all([
        db.from('test_attempts').select('*').order('submitted_at', { ascending: false }),
      ...
      const usersList = (userRes.data || []) as any[];

      const mappedSubmissions: StudentSubmission[] = rawAttempts.map((att) => {
        const u = usersList.find(usr => usr.id === att.user_id || usr.email === att.user_email);
        return {
          id: att.id || att._id?.toString() || Math.random().toString(),
          student_name: u?.full_name || att.user_email?.split('@')[0] || 'Anonymous',
          student_email: att.user_email || '',
          test_id: att.test_id || '',
          test_title: att.test_title || '',
          score: att.score || 0,
          total_marks: att.total_marks || 100,
          pass_marks: att.pass_marks || 50,
          time_taken: att.time_spent ? Math.round(att.time_spent / 60) : 0,
