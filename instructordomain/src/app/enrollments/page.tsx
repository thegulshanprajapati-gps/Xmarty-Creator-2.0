...
export default function EnrollmentsPage() {
  ...
  const fetchActor = async () => {
    try {
      ...
      if (user) {
        ...
        if (role === 'student') {
          setPermissions({ canEnroll: false, canUnenroll: false, canBulkEnroll: false });
        } else if (role === 'instructor') {
          // Instructors have limits but can allot
  ...
  const validateEnrollment = (
    ...
    targetCourses.forEach(cid => {
      ...
      if (!courseExists) {
        messages.push(`Course ID "${cid}" does not exist in DB metadata.`);
      }
    ...
    targetTests.forEach(tid => {
      ...
      if (!testExists) {
        messages.push(`Test ID "${tid}" does not exist in DB metadata.`);
      }
  ...
  const saveSingleEnrollments = async () => {
    ...
    if (!permissions.canEnroll) {
      toast({ variant: 'destructive', title: 'Unauthorized', description: 'You do not have permission to manage enrollments.' });
      return;
    }
  ...
  const saveBulkEnrollments = async () => {
    ...
    if (!permissions.canBulkEnroll) {
      toast({ variant: 'destructive', title: 'Unauthorized', description: 'You do not have permission for bulk operations.' });
      return;
    }
  ...
  const newStudentsCount = students.filter(s => {
    // Parse Date safely (we can split or check locale string)
    ...
    <SidebarProvider>
      ...
      <SidebarInset className="bg-muted/10 pb-20 relative">
        ...
        {showUndoBanner && lastAction && (
          <div className="sticky top-0 z-50 bg-amber-500 text-amber-950 font-bold px-6 py-3 flex items-center justify-between shadow-md transition-all duration-300 animate-in fade-in-50">
            <div className="flex items-center gap-2 text-sm">
              <Undo className="h-4 w-4" />
              <span>Enrollment updated. Want to revert this action?</span>
            </div>
            <Button size="sm" variant="outline" className="border-amber-950 bg-amber-950/10 hover:bg-amber-950/20 text-amber-950 font-extrabold" onClick={triggerUndo}>
              Undo Action
            </Button>
          </div>
        )}
        ...
        <header className="flex h-16 shrink-0 items-center gap-4 border-b bg-background px-6 border-amber-200/40 dark:border-amber-500/10">
          ...
          <div className="flex-1 min-w-0">
            <h1 className="font-headline font-bold text-xl">Student Enrollments</h1>
            <p className="text-xs text-muted-foreground">Premium Enterprise Dashboard: Allot courses and test assessments with audit logs and bulk triggers.</p>
          </div>
          <div className="flex gap-2">
            <Button variant="outline" size="sm" onClick={fetchData} disabled={loading}>
              <RefreshCw className={`h-4 w-4 mr-2 ${loading ? 'animate-spin' : ''}`} /> Refresh
            </Button>
            <Button size="sm" className="bg-amber-500 hover:bg-amber-600 text-amber-950 font-bold" onClick={() => {
              setDialogStudentId('');
              setDialogCourses([]);
              setDialogTests([]);
              setIsEnrollDialogOpen(true);
            ...
            <CardContent className="p-6 space-y-4">
              <div className="grid grid-cols-1 md:grid-cols-12 gap-4 items-end">
                <div className="md:col-span-6 space-y-2">
                  <label className="text-xs font-bold uppercase tracking-wider text-muted-foreground ml-1">Search Directory</label>
                ...
                <div className="w-full md:col-span-3 space-y-2">
                  <label className="text-xs font-bold uppercase tracking-wider text-muted-foreground ml-1">Filter by Course</label>
                ...
                <div className="w-full md:col-span-3 space-y-2">
                  <label className="text-xs font-bold uppercase tracking-wider text-muted-foreground ml-1">Enrollment State</label>
          ...
          <Card className="rounded-[2rem] border border-border overflow-hidden bg-background">
            <CardHeader className="bg-primary/[0.02] border-b pb-4 flex flex-row justify-between items-center">
              ...
              {selectedStudentIds.length > 0 && (
                <Badge className="bg-amber-500/10 text-amber-800 border-amber-500/30 text-xs font-bold py-1 px-3">
                        ...
                        return (
                          <TableRow key={student.id} className="hover:bg-muted/10 transition-colors">
                            ...
                            <TableCell className="font-bold text-slate-800 dark:text-white py-4 flex items-center gap-2">
                              <UserCheck className="h-4 w-4 text-amber-500" /> {student.name}
                            </TableCell>
                            ...
                            <TableCell className="pr-6 text-right space-x-1.5">
                              <Button size="sm" variant="outline" className="h-8 font-bold text-xs" onClick={() => openSingleEnroll(student.id)}>
                                Manage
                              </Button>
                              <Button size="sm" variant="outline" className="h-8 font-bold text-xs text-slate-500" onClick={() => openHistoryDialog(student)}>
        ...
        <Dialog open={isEnrollDialogOpen} onOpenChange={setIsEnrollDialogOpen}>
          <DialogContent className="max-w-md rounded-2xl">
            <DialogHeader>
              <DialogTitle className="font-headline font-bold text-xl text-amber-900 dark:text-yellow-400">Manage Enrollments</DialogTitle>
            ...
            <DialogFooter className="gap-2">
              ...
              <Button className="bg-amber-500 hover:bg-amber-600 text-amber-950 rounded-xl font-bold h-11" onClick={saveSingleEnrollments} disabled={savingEnrollments}>
        ...
        <Dialog open={isBulkDialogOpen} onOpenChange={setIsBulkDialogOpen}>
          <DialogContent className="max-w-md rounded-2xl">
            <DialogHeader>
              <DialogTitle className="font-headline font-bold text-xl text-amber-900 dark:text-yellow-400">
            ...
            <DialogFooter className="gap-2">
              ...
              <Button className="bg-amber-500 hover:bg-amber-600 text-amber-950 rounded-xl font-bold h-11" onClick={saveBulkEnrollments} disabled={savingEnrollments}>
        ...
        <Dialog open={isHistoryDialogOpen} onOpenChange={setIsHistoryDialogOpen}>
          <DialogContent className="max-w-lg rounded-2xl">
            <DialogHeader>
              <DialogTitle className="font-headline font-bold text-xl text-amber-900 dark:text-yellow-400">Enrollment Log Audit Timeline</DialogTitle>
              <DialogDescription className="text-xs text-muted-foreground">
            ...
            <div className="py-4 max-h-96 overflow-y-auto space-y-4 pr-1">
              {loadingHistory ? (
                ...
                <div className="space-y-4 border-l border-amber-200 dark:border-amber-900/40 ml-3 pl-4">
                    ...
                    return (
                      <div key={idx} className="relative space-y-1">
                        ...
                        <div className="text-[10px] text-muted-foreground font-bold uppercase tracking-wider flex justify-between">
                          ...
                          <span className="text-amber-700 dark:text-yellow-500 font-extrabold uppercase">
