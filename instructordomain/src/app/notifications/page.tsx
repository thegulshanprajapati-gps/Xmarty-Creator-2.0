...
import { Bell, Send, Users, User, ArrowRight, RefreshCw } from 'lucide-react';
      ...
      <SidebarInset className="bg-muted/10 pb-20 relative">
        <header className="flex h-16 shrink-0 items-center gap-4 border-b bg-background px-6 border-amber-200/40 dark:border-amber-500/10">
          ...
          <div className="flex-1 min-w-0">
            <h1 className="font-headline font-bold text-xl">Platform Notifications</h1>
            <p className="text-xs text-muted-foreground">Broadcast messages or send direct push notifications to individual student devices.</p>
          </div>
        ...
        <main className="p-6 md:p-8 max-w-2xl mx-auto w-full space-y-6">
          <Card className="rounded-[2rem] border border-border shadow-sm bg-background overflow-hidden">
            <CardHeader className="bg-gradient-to-r from-amber-500/10 to-transparent border-b pb-4">
            ...
            <CardContent className="p-6">
              <form onSubmit={handleSendNotification} className="space-y-5">
                  ...
                  <Select value={targetType} onValueChange={(val: any) => setTargetType(val)}>
                    ...
                    <SelectContent>
                      <SelectItem value="broadcast">
                        <span className="flex items-center gap-2 font-bold"><Users className="h-4 w-4 text-amber-500" /> Broadcast to All</span>
                      </SelectItem>
                ...
                <Button
                  ...
                  className="w-full h-12 rounded-xl bg-amber-500 hover:bg-amber-600 text-amber-950 font-extrabold text-sm flex items-center justify-center gap-2 shadow-lg shadow-amber-500/10"
                >
                  {sending ? 'Dispatching...' : (
                    <>
