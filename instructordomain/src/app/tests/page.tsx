...
function TestBuilderPanel({ test, onClose, onSaved }: {
  ...
  const [testType, setTestType] = useState<'standard' | 'contest'>(test.type || 'standard');
  const [leaderboardEnabled, setLeaderboardEnabled] = useState<boolean>(test.leaderboard_enabled || false);
  ...
  useEffect(() => {
    db.from('course_folders')
      .select('id, title')
      .match({ parent_folder_id: null, course_id: 'curriculum-catalog' })
      .order('title')
  ...
  const handleSave = async (publish = false) => {
    ...
    try {
      ...
      // Broadcast notifications if leaderboard is enabled and we are publishing
      if (leaderboardEnabled && publish) {
        fetch('/api/notifications/broadcast', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({
            testId: result.data.id || result.data._id || test.id,
          ...
          <div className="rounded-2xl border bg-card p-5 space-y-4">
            ...
            <div className="space-y-3">
              ...
              <div className="grid grid-cols-2 gap-3 pt-2">
                <div className="space-y-1.5">
                  <Label className="text-[11px] font-bold uppercase tracking-wider text-muted-foreground">Test Type</Label>
                  <Select value={testType} onValueChange={(v: any) => setTestType(v)}>
                    <SelectTrigger className="h-9 text-xs">
                      <SelectValue placeholder="Select type..." />
                    </SelectTrigger>
                    <SelectContent>
                      <SelectItem value="standard">Standard Exam</SelectItem>
                      <SelectItem value="contest">Contest / Ranked Test</SelectItem>
                    </SelectContent>
                  </Select>
                </div>
                <div className="space-y-1.5 flex flex-col justify-end pb-2">
                  <div className="flex items-center gap-2">
                    ...
                    />
                    <Label htmlFor="leaderboard_enabled" className="text-[11px] font-bold uppercase tracking-wider text-muted-foreground cursor-pointer select-none">
                      Enable Leaderboard / Rankings
                    </Label>
                  </div>
                </div>
              </div>
        ...
        <div className="flex items-start justify-between gap-3">
          <div className="min-w-0">
            <div className="flex items-center gap-2 flex-wrap">
              ...
              {test.type === 'contest' && (
                <Badge className="text-[10px] px-2 py-0.5 bg-amber-500/10 text-amber-600 dark:text-amber-400 border border-amber-500/30">
                  🏆 Contest
                </Badge>
              )}
              {test.leaderboard_enabled && (
                <Badge className="text-[10px] px-2 py-0.5 bg-emerald-500/10 text-emerald-600 dark:text-emerald-400 border border-emerald-500/30">
                  📊 Leaderboard Live
                </Badge>
              )}
            </div>
