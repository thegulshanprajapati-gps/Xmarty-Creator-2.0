...
export const GOOGLE_FONTS = [
  { label: 'Default (Inherited)', value: 'default' },
...
function loadGoogleFont(fontFamily: string) {
  if (!fontFamily || fontFamily === 'default' || typeof window === 'undefined') return;
...
export function FontColorPicker({ font, color, onChange, previewText = 'Sample Text' }: FontColorPickerProps) {
  ...
  const [localFont, setLocalFont] = useState(font || 'default');
  const [localColor, setLocalColor] = useState(color || '#0f172a');

  // Sync with external changes
  useEffect(() => {
    setLocalFont(font || 'default');
    setLocalColor(color || '#0f172a');
  }, [font, color]);

  // Load font when picker opens or font changes
  useEffect(() => {
    if (localFont && localFont !== 'default') loadGoogleFont(localFont);
  }, [localFont]);

  const handleApply = () => {
    onChange(localFont === 'default' ? '' : localFont, localColor);
    setOpen(false);
  };

  const handleReset = () => {
    setLocalFont('default');
  ...
  const displayFont = (font && font !== 'default') ? font : undefined;
      ...
      >
        ...
        <div className="p-4 space-y-4">
          ...
          <div
            ...
            style={{ fontFamily: (localFont && localFont !== 'default') ? localFont : undefined, color: localColor }}
          >
            {previewText}
          </div>
          ...
          {/* Font Family */}
            <Select value={localFont} onValueChange={(val) => setLocalFont(val)}>
              <SelectTrigger 
                ...
                style={{ fontFamily: (localFont && localFont !== 'default') ? localFont : undefined }}
              >
              ...
              <SelectContent className="max-h-[200px]">
                {GOOGLE_FONTS.map((f) => (
                  <SelectItem 
                    key={f.value} 
                    value={f.value} 
                    style={{ fontFamily: (f.value && f.value !== 'default') ? f.value : undefined }}
