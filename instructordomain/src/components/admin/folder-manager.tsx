...
export function FolderManager({ courseId, title, description, onEditCourse, onAdvanceEdit }: FolderManagerProps) {
  ...
  const showAlert = (msg: string, t: string = 'Notification') => {
    setAlertDialog({ open: true, title: t, message: msg });
  };
  ...
  const explorerItems = useMemo<ExplorerItem[]>(() => {
    const folderItems = folders.map(f => {
      ...
      return {
        ...
        category: f.category || 'Programming',
        ...
        thumbnailUrl: f.thumbnail_url || autoThumbnail || '',
  ...
  const handleCreateFolder = async (name: string, desc: string, isPaid: boolean, parentId: string | null) => {
    ...
    if (error) {
      showAlert(`Unable to create folder: ${error.message}`, 'Error');
    } else {
  ...
  const handleRenameItem = async (id: string, isFolder: boolean, newName: string) => {
    ...
    if (error) {
      showAlert(`Rename failed: ${error.message}`, 'Error');
    } else {
      await fetchStructure();
    }
    setSaving(false);
  ...
  const handleUploadFile = async (file: File, parentFolderId: string | null) => {
    ...
    if (isThumbnailFolder) {
      ...
      if (!allowedTypes.includes(file.type)) {
        showAlert("Only JPG, JPEG, PNG, or WEBP images can be uploaded as thumbnails.", "Invalid Image");
        return;
      }
      if (file.size > 3 * 1024 * 1024) {
        showAlert("Thumbnail image size must be less than 3MB.", "Size Limit Exceeded");
        return;
      }
  ...
  const handleSaveProperties = async (id: string, metadata: {
    ...
    if (error) {
      showAlert(`Failed to save folder properties: ${error.message}`, 'Error');
    } else {
      await fetchStructure();
    }
    setSaving(false);
  ...
  const handleSelectLibraryAsset = async (url: string, name: string, parentFolderId: string | null) => {
    ...
    if (isThumbnailFolder) {
      ...
      if (!allowedExts.test(url)) {
        showAlert("Only JPG, JPEG, PNG, or WEBP images can be selected as thumbnails.", "Invalid Image");
        return;
      }
      // Delete existing thumbnail records to enforce only one thumbnail
      const oldThumbnails = contents.filter(c => c.folder_id === parentFolderId);
    ...
    try {
      ...
      showAlert(`Asset selection failed: ${error.message}`, 'Selection Error');
    } finally {
      setSaving(false);
    }
  };
    ...
    <Card className="shadow-2xl border-primary/10 rounded-[2rem] overflow-hidden">
      ...
      <CardContent className="p-6">
        ...
        <WindowsExplorer
          ...
          onDeleteItems={(courseId === 'curriculum-catalog' && !(userRole === 'admin' || userRole === 'super_admin' || userRole === 'superadmin')) ? undefined : handleDeleteItems}
