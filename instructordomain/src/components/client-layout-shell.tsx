...
export function ClientLayoutShell({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    ...
    window.fetch = async (input, init) => {
      ...
      if (typeof input === 'string') {
        ...
        url = 'url' in input ? (input as Request).url : '';
      }
  ...
  useEffect(() => {
    ...
    let title = "Instructor Console";
    if (pathname === '/login') {
      title = "Sign In | Instructor Console";
    } else if (pathname === '/') {
      title = "Dashboard | Instructor Console";
    } else if (pathname === '/assessments/certificates') {
      title = "Allotted Certificates | Instructor Console";
    } else if (pathname === '/assessments/student-reports') {
      title = "Student Performance Reports | Instructor Console";
    } else if (pathname === '/tests') {
      title = "Test Management | Instructor Console";
    } else if (pathname === '/courses') {
      title = "My Courses | Instructor Console";
    } else if (pathname === '/enrollments') {
      title = "Enrollments | Instructor Console";
    } else if (pathname === '/analytics') {
      title = "Performance Analytics | Instructor Console";
    } else {
      const parts = pathname.split('/').filter(Boolean);
      if (parts.length > 0) {
        const capitalized = parts.map(p => p.charAt(0).toUpperCase() + p.slice(1).replace(/-/g, ' ')).join(' - ');
        title = `${capitalized} | Instructor Console`;
      }
    }
